...
* `feature_list.json`: ordered task queue and state machine. The first item with `"passes": false` is the next dispatch target. Every task should include `task_type` and `budget_minutes`.
* `codex-progress.md`: current summary plus recent high-signal entries for default startup.
...
2. If `feature_list.json` is empty, placeholder-only, or stale for the current requirement, the human first invokes `$task-init` to regenerate the queue.
...
10. The parent agent dispatches one worker per task in the current dependency layer, passing each worker only its task, typed execution contract, validation steps, expected write scope when inferable, and escalation contract.
...
18. After validation passes, the worker updates its assigned task in `feature_list.json`, updates `agent-state.md` when relevant, and appends a matching record to `codex-progress.md`.
19. The parent agent verifies and reconciles shared memory files after each dependency layer.
...
* Each worker owns exactly one `feature_list.json` item.
