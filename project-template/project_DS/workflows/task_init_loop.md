...
5. 判断本次队列更新模式：
   ...
   * `rewrite-pending`：保留已完成 active 任务，归档被替换的未完成任务，再写入重新规划的未完成队列，
...
* 除非人类明确要求，否则不要覆盖已完成项。
* 追加阶段任务时不要清空 active 队列；只有 `replace-active` 模式才清空后重写。
...
1. 用 `$Task_init` 生成或更新 `feature_list.json`。
2. 人类快速检查任务顺序和粒度。
...
```text
$Task_init 根据“主播考勤导入和周结算”需求拆成 12 个任务并写入 feature_list.json
```

```text
$Task_init 根据最新 PRD 重写 feature_list.json 中未完成项，保留已完成项
```
